    ...
    return {
        ...
        "shields": {
            "content_shield": {
                ...
                "detector_params": {"temperature": 0.5, "risk_name": "content_risk"},
            },
            "chat_shield": {
                "type": "chat",
                ...
                "detectors": {
                    "sub_detector_1": {"detector_params": {"param1": "value1"}}
                },
            },
        },
    }
...
class TestDetectorParams:
    ...
    def test_detectors_handling(self, sample_detector_params):
        detectors_config = {
            "detector1": {"detector_params": {"param1": "value1", "param2": "value2"}},
            "detector2": {"detector_params": {"param3": "value3"}},
        }
...
class TestBaseDetectorConfig:
    ...
    def test_message_types_validation(self):
        ...
        config = BaseDetectorConfig(
            detector_id="test", message_types=["user", "completion"]
        )
        assert config.message_types == {"user", "completion"}

        # Invalid message types
        with pytest.raises(ValueError):
            BaseDetectorConfig(detector_id="test", message_types=["invalid_type"])

    def test_url_validation(self):
        # Valid URLs
        config = BaseDetectorConfig(
            detector_id="test", detector_url="https://api.example.com/detector"
        )
        config.validate()  # Should not raise

        # Invalid URL format
        with pytest.raises(ValueError, match="Invalid detector_url format"):
            config = BaseDetectorConfig(detector_id="test", detector_url="not-a-url")
            config.validate()
    ...
    def test_auth_headers(self):
        config = BaseDetectorConfig(detector_id="test", auth_token="test_token_123")
    ...
    def test_use_orchestrator_api(self):
        config = BaseDetectorConfig(
            detector_id="test", orchestrator_url="https://api.example.com/orchestrator"
        )
        ...
        config_data = {
            "shields": {
                "test_shield": {
                    "type": "content",
                    "detector_url": "https://detector.example.com",
                    "detector_params": {"temperature": 0.8, "risk_name": "test_risk"},
                    "detectors": {
                        "sub_detector": {"detector_params": {"sub_param": "sub_value"}}
                    },
                }
            }
        }
