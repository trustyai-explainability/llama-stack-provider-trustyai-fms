...
class TestContentDetector:
    ...
    def content_config(self):
        return ContentDetectorConfig(
            ...
            detector_params=DetectorParams(
                temperature=0.5, risk_name="test_risk", regex=["test.*pattern"]
            ),
        )
    ...
    async def test_detect_content(self, content_detector, mock_shield_store):
        mock_response_data = [
            {
                "detections": [
                    {"score": 0.8, "label": "label", "detection_type": "content"}
                ]
            }
        ]

        with patch.object(
            content_detector, "_make_request", new_callable=AsyncMock
        ) as mock_request:
...
class TestChatDetector:
    ...
    def chat_config(self):
        return ChatDetectorConfig(
            ...
            detector_params=DetectorParams(
                temperature=0.7,
                detectors={"detector_1": {"detector_params": {"param1": "value1"}}},
            ),
        )
    ...
    def test_init_invalid_config(self):
        ...
        with pytest.raises(
            DetectorValidationError,
            match="Config must be an instance of ChatDetectorConfig",
        ):
            ChatDetector(content_config)

    @pytest.mark.asyncio
    async def test_detect_chat_orchestrator_mode(
        self, chat_detector, mock_chat_shield_store
    ):
        mock_response_data = {
            "detections": [
                {
        ...
        with patch.object(
            chat_detector, "_make_request", new_callable=AsyncMock
        ) as mock_request:
            mock_request.return_value = mock_response_data
    ...
    async def test_detect_chat_direct_mode(self):
        ...
        mock_response_data = [
            {
                "detections": [
                    {"score": 0.9, "label": "RISK_DETECTED", "detection_type": "chat"}
                ]
            }
        ]
        ...
        with patch.object(
            detector, "_make_request", new_callable=AsyncMock
        ) as mock_request:
            mock_request.return_value = mock_response_data
    ...
    def test_prepare_chat_request_direct(self):
        config = ChatDetectorConfig(
            detector_id="direct_chat", detector_url="https://api.example.com/chat"
        )
        detector = ChatDetector(config)
...
class TestDetectorHTTP:
    ...
    async def test_http_request_success(self, initialized_detector, httpx_mock):
        ...
        httpx_mock.add_response(
            ...
            json=[{"score": 0.8, "label": "label", "detection_type": "content"}],
            status_code=200,
        )
    ...
    async def test_http_request_no_violation(self, initialized_detector, httpx_mock):
        ...
        httpx_mock.add_response(
            method="POST",
            url="https://api.example.com/content/api/v1/text/contents",
            json=[{"score": 0.3, "label": "label", "detection_type": "content"}],
            status_code=200,
        )
        ...
        )  # Should not detect violation because score < threshold
